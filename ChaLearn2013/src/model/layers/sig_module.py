...
class SigNet(nn.Module):
    def __init__(self, in_channels, out_dimension, sig_depth, include_time=True, use_linear=True):
...
class SigModuleParallel_cheng(nn.Module):
    def __init__(self, in_channels, sig_in_channels, out_dimension, sig_depth, \
        win_size=5, use_bottleneck=False, specific_path=None, spatial_ps=False, use_linear=True):
        ...
        if self.use_bottleneck:
            self.sig = SigNet(self.sig_in_channels, self.out_dimension, self.sig_depth, use_linear=self.use_linear, \
            include_time=False if self.spatial_ps else True)
        else:
            self.sig = SigNet(self.in_channels, self.out_dimension, self.sig_depth, use_linear=self.use_linear,\
            include_time=False if self.spatial_ps else True)
    ...
    def forward(self, x):
        ...
        if self.specific_path is None:
            if self.use_bottleneck:
                ...
                self.input = torch.zeros([J*T*N, self.win_size, self.in_channels], dtype=torch.float, requires_grad=True).to(x.device)
        else:
            self.input = torch.zeros([J*T*N, len(self.specific_path), C], dtype=torch.float, requires_grad=True).to(x.device)
...
class SigModuleParallel_cheng_origin(nn.Module):
    def __init__(self, in_channels, sig_in_channels, out_dimension, sig_depth, \
        win_size=5, use_bottleneck=False, specific_path=None, spatial_ps=False, use_linear=False):
        ...
        super(SigModuleParallel_cheng_origin, self).__init__()
    ...
    def forward(self, x):
        ...
        self.res = torch.zeros([N, 37059, J, T], dtype=torch.float, requires_grad=True).to(x.device)
        ...
        if self.spatial:
            ...
            for i in range(J):
                spatial_path[B * T * i: B * T * (i + 1), :, :] = torch.index_select(x, dim=2, \
                    index=torch.from_numpy(chalearn13_hand_crafted_spatial_path[i, :].ravel()).long().to(x.device)).permute((0, 3, 2, 1)).reshape((B * T, 3, C))
...
class STEM_origin(nn.Module):
    def __init__(self, spatial, temporal, conv, C_in, C_out, with_bn=True, bn_before_actfn=False):
        super(STEM_origin, self).__init__()
        self.spatial = spatial
        self.temporal = temporal
        self.conv = conv
        ...
        if self.spatial or self.temporal:
            ...
            if self.spatial:
                self.spatial_sig = SigNet(C_in, C_out, sig_depth=3, include_time=True, use_linear=False)
            if self.temporal:
                self.temporal_sig = SigNet(C_in, C_out, sig_depth=3, include_time=True, use_linear=False)
    ...
    def forward(self, x):
        ...
        if self.spatial:
            ...
            spatial_sig = self.spatial_sig(spatial_path)
            spatial_sig = torch.Tensor.reshape(spatial_sig, (J, B, T, 84)).permute((1, 3, 0, 2))
        ...
        if self.temporal:
            ...
            temporal_sig = self.temporal_sig(temporal_path)
            temporal_sig = torch.Tensor.reshape(temporal_sig, (T, B, J, 84)).permute((1, 3, 2, 0))
...
class STEM2(nn.Module):
    def __init__(self, spatial, temporal, conv, C_in, C_out, with_bn=True, bn_before_actfn=False, use_bottleneck=True, use_linear=True):
        ...
        self.C_in = C_in
        self.C_out = C_out
        self.sig_channels = signatory.signature_channels(channels=4, depth=3)

        if self.use_bottleneck:
            self.spatial_bottleneck = nn.Conv2d(3, 3, kernel_size=(1, 1), padding=(0, 0))
            self.temporal_bottleneck = nn.Conv2d(3, 3, kernel_size=(1, 1), padding=(0, 0))
        else:
            self.spatial_bottleneck = nn.Identity()
            self.temporal_bottleneck = nn.Identity()
        ...
        if self.spatial or self.temporal:
            ...
            if self.spatial and self.temporal:
                ...
                if self.use_linear:
                    ...
                    self.ps_fusion.add_module("conv", nn.Conv2d(int(self.sig_channels * 2), C_out, kernel_size=(1, 1), padding=(0, 0)))
            ...
            if self.spatial:
                self.spatial_sig = SigNet(C_in, C_out, sig_depth=3, include_time=True, use_linear=self.use_linear)
            if self.temporal:
                self.temporal_sig = SigNet(C_in, C_out, sig_depth=3, include_time=True, use_linear=self.use_linear)
    ...
    def forward(self, x):
        ...
        if self.spatial:
            x_spatial = self.spatial_bottleneck(x)
            spatial_path = torch.zeros((int(B * J * T), 3, C), dtype=x.dtype, device=x.device)
            for i in range(J):
                spatial_path[B * T * i: B * T * (i + 1), :, :] = torch.index_select(x_spatial, dim=2, \
                    index=torch.from_numpy(chalearn13_hand_crafted_spatial_path[i, :].ravel()).long().to(x.device)).permute((0, 3, 2, 1)).reshape((B * T, 3, C))
            spatial_path = spatial_path.contiguous()
            ...
            if self.use_linear:
                ...
                spatial_sig = torch.Tensor.reshape(spatial_sig, (J, B, T, self.sig_channels)).permute((1, 3, 0, 2))

        if self.temporal:
            x_temporal = self.temporal_bottleneck(x)
            temporal_path = torch.zeros((int(B * J * T), 3, C), dtype=x.dtype, device=x.device)
            x_ps = torch.nn.functional.pad(x_temporal, (1, 1, 0, 0), 'replicate')
            for i in range(T):
                temporal_path[(B * J) * i:(B * J) * (i + 1), :, :] = x_ps[:, :, :, i:i + 3].permute((0, 2, 3, 1)).reshape((B * J, 3, C))
            temporal_path = temporal_path.contiguous()
            ...
            if self.use_linear:
                ...
                temporal_sig = torch.Tensor.reshape(temporal_sig, (T, B, J, self.sig_channels)).permute((1, 3, 2, 0))
