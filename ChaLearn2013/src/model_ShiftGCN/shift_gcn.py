...
from src.model_ShiftGCN.Temporal_shift.cuda.shift import Shift
...
class Shift_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, num_subset=3, num_point=11):
        ...
        self.Feature_Mask = nn.Parameter(torch.ones(1,self.num_point,in_channels, requires_grad=True,device='cuda'),requires_grad=True)
        ...
        index_array = np.empty(self.num_point*in_channels).astype(np.int)
        for i in range(self.num_point):
            for j in range(in_channels):
                index_array[i*in_channels + j] = (i*in_channels + j + j*in_channels)%(in_channels*self.num_point)
        self.shift_in = nn.Parameter(torch.from_numpy(index_array),requires_grad=False)

        index_array = np.empty(self.num_point*out_channels).astype(np.int)
        for i in range(self.num_point):
            for j in range(out_channels):
                index_array[i*out_channels + j] = (i*out_channels + j - j*out_channels)%(out_channels*self.num_point)
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, num_point=11):
...
class ShiftGCN_Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, in_channels=3):
        ...
        self.l1 = TCN_GCN_unit(in_channels, 64, A, residual=False, num_point=num_point)
