...
#         x = np.concatenate((x[:, :13, :, :C_in], x[:, 17:, :, :C_in]), axis=1)
#         # pdb.set_trace()
...
# def sel_temp_point(batch_xs, sel_temp_points_id=[i for i in range(55)], sel_points_id=[i for i in range(55)], final_frame_nb=39):
...
# 			batch_xs[:,p_id*(final_frame_nb*C_in):(p_id+1)*(final_frame_nb*C_in)], axis=1)
