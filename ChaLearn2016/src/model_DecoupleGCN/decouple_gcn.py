...
import numpy as np
import math
from src.model_DecoupleGCN.dropSke import DropBlock_Ske
from src.model_DecoupleGCN.dropT import DropBlockT_1d
...
class unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1, num_point=11):
...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, coff_embedding=4, num_subset=3, num_point=11):
        ...
        self.DecoupleA = nn.Parameter(torch.tensor(np.reshape(A.astype(np.float32),[3,1,self.num_point,self.num_point]), dtype=torch.float32, requires_grad=True).repeat(1,groups,1,1), requires_grad=True)
...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, stride=1, residual=True, num_point=11):
        super(TCN_GCN_unit, self).__init__()
        self.gcn1 = unit_gcn(in_channels, out_channels, A, groups, num_point=num_point)
        self.tcn1 = unit_tcn(out_channels, out_channels, stride=stride, num_point=num_point)
        self.relu = nn.ReLU()

        self.num_point = num_point
        self.A = nn.Parameter(torch.tensor(np.sum(np.reshape(A.astype(np.float32),[3,self.num_point,self.num_point]),axis=0), dtype=torch.float32,requires_grad=False,device='cuda'),requires_grad=False)
        ...
        self.dropSke = DropBlock_Ske(num_point=self.num_point)
...
class DecoupleGCN_Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, groups=8, graph=None, in_channels=3):
        super(DecoupleGCN_Model, self).__init__()
        ...
        self.l1 = TCN_GCN_unit(in_channels, 64, A, groups, residual=False, num_point=num_point)
        self.l2 = TCN_GCN_unit(64, 64, A, groups, num_point=num_point)
        self.l3 = TCN_GCN_unit(64, 64, A, groups, num_point=num_point)
        self.l4 = TCN_GCN_unit(64, 64, A, groups, num_point=num_point)
        self.l5 = TCN_GCN_unit(64, 128, A, groups, stride=2, num_point=num_point)
        self.l6 = TCN_GCN_unit(128, 128, A, groups, num_point=num_point)
        self.l7 = TCN_GCN_unit(128, 128, A, groups, num_point=num_point)
        self.l8 = TCN_GCN_unit(128, 256, A, groups, stride=2, num_point=num_point)
        self.l9 = TCN_GCN_unit(256, 256, A, groups, num_point=num_point)
        self.l10 = TCN_GCN_unit(256, 256, A, groups, num_point=num_point)
